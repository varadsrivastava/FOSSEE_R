#What is the probability that a patient will be referred fr further lab tests

#Sample Space
S <- sample(c("Positive", "Negative"))

#Sample space for Doctor A
A <- sample(c("Positive", "Negative"),prob = c(0.1,0.9))
#Sample space for Doctor B
A <- sample(c("Positive", "Negative"),prob = c(0.17,0.83))

#Probability that doctor A makes a positive diagnosis
PA <- 0.1
#Probability that doctor B makes a positive diagnosis
PB <- 0.17

#Probability that both doctors make a positive diagnosis
PAB <- 0.08

#function to compute union by addition law of probability
union <- function(x,y,z)
{
  result = x + y - z
  print(result)
}

#Probability that a positive diagnosis is made by either of them and both of them
PAUB = union(PA,PB,PAB)
print("Probability that a patient will be referred for further lab tests:")
PAUB